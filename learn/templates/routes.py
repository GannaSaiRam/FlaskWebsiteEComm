...
from learn.models import Item, User
from learn.forms import RegisterForm, LoginForm, SellItemForm, PurchaseItemForm
	...
	if request.method == "POST":
		...
		if p_item_object:
			if current_user.can_purchase(p_item_object):
				...
				flash(f"Congratulations! You bought {p_item_object.name} for {p_item_object.price}$", category='success')
			else:
				flash(f"Unfortunately, you don't have sufficient funds to purchase {p_item_object.name}", category="danger")
		# Sell Item Logic
...
def register_page():
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		print(user_to_create.username, user_to_create.password_hash, user_to_create.id)
...
def login_page():
	...
	if form.validate_on_submit():
		...
		if attempted_user and attempted_user.check_password_correction(attempted_password=form.password.data):
			print(attempted_user.username, attempted_user.password_hash, attempted_user.id)
